...
class MonteCarloTreeSearchDeepTree:

    def __init__(self, _id, initial_state, time_limit=THINK_TIME, exploration_constant=1 / math.sqrt(2),
                 discount_factor=0.9):
    ...
    def MABBestMove(self, node):
        ...
        for child in node.children:
            ...
            if child.num_visited == 0:
                ...
                current_player = child.state.player_id
                # UCB1 formula for move score with given exploration constant
                move_score = (child.player_scores[current_player] - child.player_scores[1 - current_player]) + \
    ...
    def simulation(self, state):
        ...
        while state_simulation.TilesRemaining():
            ...
            if first_move_0 is None and current_player == 0:
                first_move_0 = chosen_move
            if first_move_1 is None and current_player == 1:
                first_move_1 = chosen_move
        ...
        scores = [self.calculateBonus(state_simulation, lines_state_0, board_state_0, first_move_0, 0),
                  self.calculateBonus(state_simulation, lines_state_1, board_state_1, first_move_1, 1)]
        return scores
    ...
    def calculateBonus(self, azul_state, previous_lines, previous_board, first_move, player_id):
        game_ending = False
        ...
        if not game_ending:
            ...
            score = self.linesBonus(azul_state, player_id, previous_lines, first_move) + \
    ...
    def linesBonus(self, azul_state, player_id, previous_lines, first_move):
        lines_bonus = 0
        ending_lines = azul_state.agents[player_id].lines_number
        ending_tiles = azul_state.agents[player_id].lines_tile
        ...
        if previous_lines[3] == 0 and ending_lines[3] == 3:
            if azul_state.agents[player_id].grid_scheme[3][ending_tiles[3]] == 1 \
                ...
                lines_bonus += self.ROW_BONUS[1] * 0.8
            elif azul_state.agents[player_id].grid_scheme[3][ending_tiles[3]] == 2:
                lines_bonus += self.ROW_BONUS[2] * 0.8
            else:
                lines_bonus += self.ROW_BONUS[0] * 0.8
        ...
        if first_move is not None:
            lines_bonus = 0.3 * first_move[2].num_to_pattern_line
        return lines_bonus
        ...
        for i in range(azul_state.agents[player_id].GRID_SIZE):
            for j in range(1, azul_state.agents[player_id].GRID_SIZE):
                if new_tiles[i][j] == 1 and round_num <= self.EARLY_GAME:
                    tiles_bonus += self.COLUMN_BONUS[i] * self.ROW_BONUS[j]
                    tiles_bonus += self.ROW_BONUS[j]
                elif new_tiles[i][j] == 1 and round_num > self.EARLY_GAME:
                    tiles_bonus += 0.5 * self.COLUMN_BONUS[i] * self.ROW_BONUS[j]
